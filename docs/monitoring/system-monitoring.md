# System Monitoring Module

## Обзор

System Monitoring - это комплексный модуль непрерывного мониторинга системы, обеспечивающий отслеживание системных ресурсов, сетевых соединений, процессов, целостности файлов и системных логов в реальном времени.

## Архитектура

### Компоненты

```mermaid
graph TB
    A[System Resources] --> B[Resource Monitor]
    C[Network Connections] --> D[Network Monitor]
    E[Process Activity] --> F[Process Monitor]
    G[File Changes] --> H[File Integrity Monitor]
    I[System Logs] --> J[Log Monitor]
    K[Network Traffic] --> L[Traffic Monitor]
    
    B --> M[Alert Engine]
    D --> M
    F --> M
    H --> M
    J --> M
    L --> M
    
    M --> N[Security Alerts]
    M --> O[Log Storage]
```

### Основные классы

- **RSecureLogger** - основной класс мониторинга
- **Alert callbacks** - обработчики оповещений

## Конфигурация

### Параметры по умолчанию

```python
default_config = {
    'log_interval': 1,  # секунды
    'network_scan_interval': 30,  # секунды
    'file_scan_interval': 60,  # секунды
    'max_log_size': 100 * 1024 * 1024,  # 100MB
    'log_retention_days': 30,
    'monitor_paths': [
        '/etc', '/bin', '/sbin', '/usr/bin', '/usr/sbin',
        '/System/Library', '/Library', '/Applications'
    ],
    'network_ports': [22, 80, 443, 3306, 5432, 6379, 27017],
    'alert_threshold': 0.8
}
```

## Структурированное логирование

### Настройка логов

```python
def _setup_logging(self):
    """Настройка структурированного логирования"""
    # Основной лог безопасности
    self.rsecure_log = logging.getLogger('rsecure_monitor')
    self.rsecure_log.setLevel(logging.INFO)
    
    # Обработчик файла для основного лога
    main_handler = logging.FileHandler(
        self.log_dir / 'rsecure_main.log',
        mode='a',
        encoding='utf-8'
    )
    main_handler.setFormatter(logging.Formatter(
        '%(asctime)s - %(levelname)s - %(message)s'
    ))
    self.rsecure_log.addHandler(main_handler)
    
    # Сетевой лог
    self.network_log = logging.getLogger('network_monitor')
    self.network_log.setLevel(logging.INFO)
    network_handler = logging.FileHandler(
        self.log_dir / 'network_activity.log',
        mode='a',
        encoding='utf-8'
    )
    network_handler.setFormatter(logging.Formatter(
        '%(asctime)s - %(message)s'
    ))
    self.network_log.addHandler(network_handler)
    
    # Лог процессов
    self.process_log = logging.getLogger('process_monitor')
    self.process_log.setLevel(logging.INFO)
    process_handler = logging.FileHandler(
        self.log_dir / 'process_activity.log',
        mode='a',
        encoding='utf-8'
    )
    process_handler.setFormatter(logging.Formatter(
        '%(asctime)s - %(message)s'
    ))
    self.process_log.addHandler(process_handler)
    
    # Лог целостности файлов
    self.file_log = logging.getLogger('file_monitor')
    self.file_log.setLevel(logging.INFO)
    file_handler = logging.FileHandler(
        self.log_dir / 'file_integrity.log',
        mode='a',
        encoding='utf-8'
    )
    file_handler.setFormatter(logging.Formatter(
        '%(asctime)s - %(message)s'
    ))
    self.file_log.addHandler(file_handler)
```

## Мониторинг системных ресурсов

### Отслеживание CPU, памяти, диска

```python
def _monitor_system_resources(self, interval: int):
    """Мониторинг CPU, памяти, диска"""
    while self.running:
        try:
            cpu_percent = psutil.cpu_percent(interval=1)
            memory = psutil.virtual_memory()
            disk = psutil.disk_usage('/')
            
            system_stats = {
                'timestamp': datetime.now().isoformat(),
                'cpu_percent': cpu_percent,
                'memory_percent': memory.percent,
                'memory_used': memory.used,
                'memory_total': memory.total,
                'disk_percent': disk.percent,
                'disk_used': disk.used,
                'disk_total': disk.total,
                'load_average': os.getloadavg() if hasattr(os, 'getloadavg') else None
            }
            
            self.rsecure_log.info(f"SYSTEM_STATS: {json.dumps(system_stats)}")
            
            # Проверка аномалий
            if cpu_percent > 90 or memory.percent > 90:
                self._trigger_rsecure_alert('HIGH_RESOURCE_USAGE', system_stats)
                
        except Exception as e:
            self.rsecure_log.error(f"Error monitoring system resources: {e}")
        
        time.sleep(interval)
```

## Мониторинг сетевых соединений

### Отслеживание сетевых подключений

```python
def _monitor_network_connections(self, interval: int):
    """Мониторинг сетевых соединений"""
    while self.running:
        try:
            current_connections = set()
            connections = psutil.net_connections()
            
            for conn in connections:
                if conn.status == 'ESTABLISHED':
                    conn_key = (conn.laddr, conn.raddr, conn.pid)
                    current_connections.add(conn_key)
                    
                    # Логирование новых соединений
                    if conn_key not in self.network_connections:
                        conn_info = {
                            'timestamp': datetime.now().isoformat(),
                            'local_address': f"{conn.laddr.ip}:{conn.laddr.port}",
                            'remote_address': f"{conn.raddr.ip}:{conn.raddr.port}" if conn.raddr else "N/A",
                            'status': conn.status,
                            'pid': conn.pid,
                            'process_name': self._get_process_name(conn.pid) if conn.pid else "Unknown"
                        }
                        
                        self.network_log.info(f"NEW_CONNECTION: {json.dumps(conn_info)}")
                        
                        # Проверка подозрительных соединений
                        self._check_suspicious_connection(conn_info)
            
            # Проверка закрытых соединений
            closed_connections = self.network_connections - current_connections
            for conn in closed_connections:
                conn_info = {
                    'timestamp': datetime.now().isoformat(),
                    'local_address': f"{conn[0].ip}:{conn[0].port}",
                    'remote_address': f"{conn[1].ip}:{conn[1].port}" if conn[1] else "N/A",
                    'pid': conn[2],
                    'event': 'CONNECTION_CLOSED'
                }
                self.network_log.info(f"CONNECTION_CLOSED: {json.dumps(conn_info)}")
            
            self.network_connections = current_connections
            
        except Exception as e:
            self.rsecure_log.error(f"Error monitoring network connections: {e}")
        
        time.sleep(interval)
```

### Проверка подозрительных соединений

```python
def _check_suspicious_connection(self, conn_info: Dict):
    """Проверка подозрительности соединения"""
    suspicious_indicators = [
        'remote_address' in conn_info and any(
            port in conn_info['remote_address'] for port in [':4444', ':5555', ':6666', ':7777', ':8888', ':9999']
        ),
        'process_name' in conn_info and conn_info['process_name'] in [
            'nc', 'netcat', 'telnet', 'ftp', 'tftp'
        ]
    ]
    
    if any(suspicious_indicators):
        self._trigger_rsecure_alert('SUSPICIOUS_NETWORK_CONNECTION', conn_info)
```

## Мониторинг процессов

### Отслеживание создания и завершения процессов

```python
def _monitor_processes(self, interval: int):
    """Мониторинг создания и завершения процессов"""
    while self.running:
        try:
            current_processes = {}
            
            for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'cpu_percent', 'memory_percent']):
                try:
                    pinfo = proc.info
                    pid = pinfo['pid']
                    current_processes[pid] = pinfo
                    
                    # Проверка новых процессов
                    if pid not in self.process_registry:
                        process_info = {
                            'timestamp': datetime.now().isoformat(),
                            'pid': pid,
                            'name': pinfo['name'],
                            'cmdline': pinfo['cmdline'],
                            'event': 'PROCESS_STARTED',
                            'parent_pid': proc.ppid(),
                            'user': proc.username()
                        }
                        
                        self.process_log.info(f"NEW_PROCESS: {json.dumps(process_info)}")
                        
                        # Проверка подозрительных процессов
                        self._check_suspicious_process(process_info)
                
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    continue
            
            # Проверка завершенных процессов
            terminated_pids = set(self.process_registry.keys()) - set(current_processes.keys())
            for pid in terminated_pids:
                process_info = {
                    'timestamp': datetime.now().isoformat(),
                    'pid': pid,
                    'name': self.process_registry[pid].get('name', 'Unknown'),
                    'event': 'PROCESS_TERMINATED'
                }
                self.process_log.info(f"PROCESS_TERMINATED: {json.dumps(process_info)}")
            
            self.process_registry = current_processes
            
        except Exception as e:
            self.rsecure_log.error(f"Error monitoring processes: {e}")
        
        time.sleep(interval)
```

### Проверка подозрительных процессов

```python
def _check_suspicious_process(self, process_info: Dict):
    """Проверка подозрительности процесса"""
    suspicious_names = [
        'nc', 'netcat', 'telnet', 'ftp', 'tftp', 'wget', 'curl',
        'ssh', 'scp', 'rsync', 'python', 'perl', 'ruby'
    ]
    
    if process_info.get('name') in suspicious_names:
        self._trigger_rsecure_alert('SUSPICIOUS_PROCESS', process_info)
```

## Мониторинг целостности файлов

### Отслеживание изменений файлов

```python
def _monitor_file_integrity(self, interval: int):
    """Мониторинг целостности файлов в критических путях"""
    while self.running:
        try:
            for path in self.config['monitor_paths']:
                if os.path.exists(path):
                    self._scan_directory(path)
                
        except Exception as e:
            self.rsecure_log.error(f"Error monitoring file integrity: {e}")
        
        time.sleep(interval)
```

### Сканирование директории

```python
def _scan_directory(self, directory: str):
    """Сканирование директории на изменения файлов"""
    try:
        for root, dirs, files in os.walk(directory):
            for file in files:
                file_path = os.path.join(root, file)
                
                try:
                    # Расчет хеша файла
                    with open(file_path, 'rb') as f:
                        file_hash = hashlib.sha256(f.read()).hexdigest()
                    
                    file_key = file_path
                    
                    if file_key in self.file_hashes:
                        if self.file_hashes[file_key] != file_hash:
                            file_info = {
                                'timestamp': datetime.now().isoformat(),
                                'file_path': file_path,
                                'old_hash': self.file_hashes[file_key],
                                'new_hash': file_hash,
                                'event': 'FILE_MODIFIED'
                            }
                            
                            self.file_log.info(f"FILE_MODIFIED: {json.dumps(file_info)}")
                            self._trigger_alert('FILE_INTEGRITY_VIOLATION', file_info)
                    else:
                        # Новый файл
                        self.file_hashes[file_key] = file_hash
                        file_info = {
                            'timestamp': datetime.now().isoformat(),
                            'file_path': file_path,
                            'hash': file_hash,
                            'event': 'FILE_CREATED'
                        }
                        self.file_log.info(f"FILE_CREATED: {json.dumps(file_info)}")
                
                except (OSError, PermissionError):
                    continue
                    
    except Exception as e:
        self.file_log.error(f"Error scanning directory {directory}: {e}")
```

## Мониторинг системных логов

### Анализ системных логов

```python
def _monitor_system_logs(self, interval: int):
    """Мониторинг системных логов на события безопасности"""
    while self.running:
        try:
            system = platform.system()
            
            if system == 'Darwin':
                log_files = ['/var/log/system.log', '/var/log/install.log']
            elif system == 'Linux':
                log_files = ['/var/log/auth.log', '/var/log/syslog', '/var/log/messages']
            else:
                log_files = []
            
            for log_file in log_files:
                if os.path.exists(log_file):
                    self._parse_system_log(log_file)
                
        except Exception as e:
            self.rsecure_log.error(f"Error monitoring system logs: {e}")
        
        time.sleep(interval)
```

### Парсинг системных логов

```python
def _parse_system_log(self, log_file: str):
    """Парсинг системного лога на события безопасности"""
    try:
        # Упрощенная реализация
        # В продакшене использовались бы библиотеки парсинга логов
        # и отслеживание позиций в файле для избежания повторного чтения
        
        security_keywords = [
            'failed', 'denied', 'attack', 'intrusion', 'breach',
            'unauthorized', 'malware', 'virus', 'trojan', 'rootkit'
        ]
        
        with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                if any(keyword in line.lower() for keyword in security_keywords):
                    log_entry = {
                        'timestamp': datetime.now().isoformat(),
                        'source': log_file,
                        'message': line.strip(),
                        'event': 'SECURITY_LOG_ENTRY'
                    }
                    self.rsecure_log.warning(f"SECURITY_EVENT: {json.dumps(log_entry)}")
                    
    except Exception as e:
        self.rsecure_log.error(f"Error parsing system log {log_file}: {e}")
```

## Мониторинг сетевого трафика

### Статистика сетевого трафика

```python
def _monitor_network_traffic(self, interval: int):
    """Мониторинг статистики сетевого трафика"""
    while self.running:
        try:
            net_io = psutil.net_io_counters()
            interfaces = psutil.net_io_counters(pernic=True)
            
            traffic_stats = {
                'timestamp': datetime.now().isoformat(),
                'bytes_sent': net_io.bytes_sent,
                'bytes_recv': net_io.bytes_recv,
                'packets_sent': net_io.packets_sent,
                'packets_recv': net_io.packets_recv,
                'interfaces': {}
            }
            
            for interface, stats in interfaces.items():
                traffic_stats['interfaces'][interface] = {
                    'bytes_sent': stats.bytes_sent,
                    'bytes_recv': stats.bytes_recv,
                    'packets_sent': stats.packets_sent,
                    'packets_recv': stats.packets_recv
                }
            
            self.network_log.info(f"TRAFFIC_STATS: {json.dumps(traffic_stats)}")
            
        except Exception as e:
            self.rsecure_log.error(f"Error monitoring network traffic: {e}")
        
        time.sleep(interval)
```

## Создание базовой линии системы

### Базовая линия состояния системы

```python
def _create_system_baseline(self):
    """Создание базовой линии состояния системы"""
    try:
        self.system_baseline = {
            'timestamp': datetime.now().isoformat(),
            'hostname': socket.gethostname(),
            'platform': platform.platform(),
            'users': self._get_system_users(),
            'installed_packages': self._get_installed_packages(),
            'running_services': self._get_running_services(),
            'open_ports': self._get_open_ports()
        }
        
        baseline_file = self.log_dir / 'system_baseline.json'
        with open(baseline_file, 'w') as f:
            json.dump(self.system_baseline, f, indent=2)
        
        self.rsecure_log.info("System baseline created successfully")
        
    except Exception as e:
        self.rsecure_log.error(f"Error creating system baseline: {e}")
```

### Получение системной информации

```python
def _get_system_users(self) -> List[str]:
    """Получение списка системных пользователей"""
    try:
        if platform.system() == 'Darwin':
            result = subprocess.run(['dscl', '.', 'list', '/Users'], 
                                  capture_output=True, text=True)
        else:
            result = subprocess.run(['cat', '/etc/passwd'], 
                                  capture_output=True, text=True)
        
        return result.stdout.strip().split('\n')
    except:
        return []

def _get_installed_packages(self) -> List[str]:
    """Получение списка установленных пакетов"""
    try:
        if platform.system() == 'Darwin':
            result = subprocess.run(['brew', 'list'], capture_output=True, text=True)
        else:
            result = subprocess.run(['dpkg', '-l'], capture_output=True, text=True)
        
        return result.stdout.strip().split('\n')
    except:
        return []

def _get_running_services(self) -> List[str]:
    """Получение списка запущенных сервисов"""
    try:
        if platform.system() == 'Darwin':
            result = subprocess.run(['launchctl', 'list'], capture_output=True, text=True)
        else:
            result = subprocess.run(['systemctl', 'list-units', '--type=service', '--state=running'], 
                                  capture_output=True, text=True)
        
        return result.stdout.strip().split('\n')
    except:
        return []

def _get_open_ports(self) -> List[str]:
    """Получение списка открытых портов"""
    try:
        result = subprocess.run(['netstat', '-tuln'], capture_output=True, text=True)
        return result.stdout.strip().split('\n')
    except:
        return []
```

## Система оповещений

### Генерация оповещений

```python
def _trigger_rsecure_alert(self, alert_type: str, data: Dict):
    """Генерация оповещения безопасности"""
    alert = {
        'timestamp': datetime.now().isoformat(),
        'type': alert_type,
        'data': data,
        'severity': self._get_alert_severity(alert_type)
    }
    
    self.rsecure_log.warning(f"ALERT: {json.dumps(alert)}")
    
    # Вызов callback функций оповещений
    for callback in self.alert_callbacks:
        try:
            callback(alert)
        except Exception as e:
            self.rsecure_log.error(f"Error in alert callback: {e}")
```

### Уровни серьезности

```python
def _get_alert_severity(self, alert_type: str) -> str:
    """Получение уровня серьезности оповещения"""
    severity_map = {
        'HIGH_RESOURCE_USAGE': 'medium',
        'SUSPICIOUS_NETWORK_CONNECTION': 'high',
        'SUSPICIOUS_PROCESS': 'high',
        'FILE_INTEGRITY_VIOLATION': 'critical',
        'SECURITY_LOG_ENTRY': 'medium'
    }
    return severity_map.get(alert_type, 'low')
```

### Callback функции оповещений

```python
def add_alert_callback(self, callback: Callable):
    """Добавление callback функции оповещений"""
    self.alert_callbacks.append(callback)
```

## Получение логов

### Получение последних записей

```python
def get_recent_logs(self, log_type: str = 'security', lines: int = 100) -> List[str]:
    """Получение последних записей лога"""
    log_files = {
        'rsecure': 'rsecure_main.log',
        'network': 'network_activity.log',
        'process': 'process_activity.log',
        'file': 'file_integrity.log'
    }
    
    if log_type not in log_files:
        return []
    
    log_file = self.log_dir / log_files[log_type]
    
    try:
        with open(log_file, 'r') as f:
            all_lines = f.readlines()
            return all_lines[-lines:] if len(all_lines) > lines else all_lines
    except:
        return []
```

## Интеграция с RSecure

### Инициализация в основной системе

```python
# В RSecureMain
def initialize_components(self):
    """Инициализация компонентов RSecure"""
    if self.config['system_monitoring']['enabled']:
        self.monitoring_logger = RSecureLogger(
            log_dir=self.config['system_monitoring']['log_dir'],
            config=self.config['system_monitoring']
        )
        self.monitoring_logger.start_monitoring()
        self.logger.info("System monitoring initialized")
```

### Обработка результатов

```python
def _process_monitoring_status(self, status: Dict):
    """Обработка статуса системного мониторинга"""
    # Добавление callback для оповещений
    def handle_alert(alert):
        self.logger.warning(f"Security alert: {alert['type']} - {alert['severity']}")
        self.metrics['security_alerts'] += 1
    
    self.monitoring_logger.add_alert_callback(handle_alert)
```

## Преимущества подхода

### 1. Комплексный мониторинг

- **Многопоточный** - одновременный мониторинг различных аспектов
- **Реальное время** - мгновенное обнаружение изменений
- **Структурированные логи** - удобный анализ и поиск

### 2. Интеллектуальная детекция

- **Аномалии ресурсов** - обнаружение необычной нагрузки
- **Подозрительные соединения** - мониторинг сетевой активности
- **Целостность файлов** - отслеживание изменений в критических файлах

### 3. Гибкость

- **Настраиваемые пути** - мониторинг необходимых директорий
- **Адаптивные интервалы** - настройка частоты проверки
- **Callback система** - интеграция с другими компонентами

### 4. Надежность

- **Обработка ошибок** - устойчивость к сбоям
- **Ротация логов** - управление размером логов
- **Базовая линия** - эталонное состояние системы

## Использование

### Базовый пример

```python
# Создание монитора
logger = RSecureLogger(log_dir="/var/log/security_monitor")

def print_alert(alert):
    print(f"ALERT: {alert['type']} - {alert['severity']}")

logger.add_alert_callback(print_alert)
logger.start_monitoring()

try:
    while True:
        # Получение последних логов
        recent_logs = logger.get_recent_logs('security', 50)
        for log in recent_logs[-5:]:
            print(log.strip())
        
        time.sleep(10)
except KeyboardInterrupt:
    logger.stop_monitoring()
```

### Анализ логов

```python
# Получение сетевых логов
network_logs = logger.get_recent_logs('network', 100)
for log in network_logs:
    if 'NEW_CONNECTION' in log:
        print(f"New connection: {log}")

# Получение логов процессов
process_logs = logger.get_recent_logs('process', 50)
for log in process_logs:
    if 'PROCESS_STARTED' in log:
        print(f"Process started: {log}")
```

---

System Monitoring обеспечивает всесторонний контроль над системой, обеспечивая раннее обнаружение угроз и аномалий через непрерывный мониторинг всех ключевых аспектов системной активности.
