...
class ReportGenerator {
    ...
    async generate(result, format = 'json', reportConfig) {
    ...
    generateJSONReport(result, config) {
        ...
        const report = {
            metadata: {
                ...
                filesAnalyzed: result.statistics.filesAnalyzed
            },
...
<body>
    ...
    async generatePDFReport(result, config) {
        const filteredResult = this.filterResult(result, config);
    ...
    generateSARIFReport(result, config) {
        const filteredResult = this.filterResult(result, config);
        const sarifLog = {
            version: '2.1.0',
            ...
            runs: [
                {
                    ...
                    statistics: {
                        ...
                        bySeverity: {
                            critical: filteredResult.vulnerabilities.filter(v => v.severity === types_1.Severity.CRITICAL).length,
                            high: filteredResult.vulnerabilities.filter(v => v.severity === types_1.Severity.HIGH).length,
                            medium: filteredResult.vulnerabilities.filter(v => v.severity === types_1.Severity.MEDIUM).length,
                            low: filteredResult.vulnerabilities.filter(v => v.severity === types_1.Severity.LOW).length
                        }
                    }
                }
            ]
    ...
    generateSARIFResult(vulnerability) {
        return {
                    ...
                    regions: [
                        {
                            ...
                            startColumn: vulnerability.column,
                            snippet: {
                                text: vulnerability.codeExample
                            }
                        }
                    ]
                }
    ...
    mapSeverityToSARIFLevel(severity) {
        const mapping = {
            [types_1.Severity.CRITICAL]: 'error',
            [types_1.Severity.HIGH]: 'error',
            [types_1.Severity.MEDIUM]: 'warning',
            [types_1.Severity.LOW]: 'note',
            [types_1.Severity.INFO]: 'note'
        };
    ...
    generateSummary(result) {
        ...
        return {
            ...
            bySeverity: {
                critical: vulnerabilities.filter(v => v.severity === types_1.Severity.CRITICAL).length,
                high: vulnerabilities.filter(v => v.severity === types_1.Severity.HIGH).length,
                medium: vulnerabilities.filter(v => v.severity === types_1.Severity.MEDIUM).length,
                low: vulnerabilities.filter(v => v.severity === types_1.Severity.LOW).length
            },
            byType: this.groupByType(vulnerabilities),
            filesAnalyzed: result.statistics.filesAnalyzed,
            queriesAnalyzed: result.statistics.totalQueries,
            coverage: result.statistics.coverage
        };
    }
    groupVulnerabilities(vulnerabilities, groupBy) {
        switch (groupBy) {
            case 'type':
    ...
    generateHTMLVulnerabilities(groupedVulnerabilities, config) {
        let html = '';
    ...
    generateHTMLVulnerability(vulnerability, config) {
    ...
    async generateJIRATickets(result, config = {}) {
    ...
    getHighestSeverity(vulnerabilities) {
        const severityOrder = [types_1.Severity.CRITICAL, types_1.Severity.HIGH, types_1.Severity.MEDIUM, types_1.Severity.LOW];
    ...
    shouldCreateTicket(severity) {
        return severity === types_1.Severity.CRITICAL || severity === types_1.Severity.HIGH;
    }
    mapToJIRAPriority(severity) {
        const mapping = {
            [types_1.Severity.CRITICAL]: 'Highest',
            [types_1.Severity.HIGH]: 'High',
            [types_1.Severity.MEDIUM]: 'Medium',
            [types_1.Severity.LOW]: 'Low',
            [types_1.Severity.INFO]: 'Lowest'
        };
        return mapping[severity];
    }
    generateJIRADescription(vulnerabilities) {
        const description = `
...
This ticket was automatically generated by the SQL Vulnerability Scanner.
    ...
    generateExecutiveSummary(result) {
        const criticalCount = result.vulnerabilities.filter(v => v.severity === types_1.Severity.CRITICAL).length;
        const highCount = result.vulnerabilities.filter(v => v.severity === types_1.Severity.HIGH).length;
...
- **Security Coverage:** ${result.statistics.coverage.toFixed(1)}%
