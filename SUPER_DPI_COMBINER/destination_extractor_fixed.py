#!/usr/bin/env python3
"""
Cross-Platform Original Destination Extractor - Исправленная версия
"""
...
class CrossPlatformDestinationExtractor:
    ...
    def _extract_macos_destination(self, sock):
        ...
        try:
            # На macOS нужно установить соединение перед извлечением
            # Получаем peer информацию
            peername = sock.getpeername()
            self.logger.info(f"macOS peer info: {peername}")
            
            # Для macOS используем IP_PKTINFO
            import fcntl
            import os
    ...
    def test_destination_extraction(self):
        ...
        try:
            ...
            # Устанавливаем соединение для теста
            if self.platform == "Linux":
                sock.connect(("93.184.216.34", 80))  # example.com
            elif self.platform == "Darwin":
                # На macOS используем локальный адрес для теста
                sock.connect(("127.0.0.1", 8080))  # local test
