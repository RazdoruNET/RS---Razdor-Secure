#!/usr/bin/env python3
"""
DPI-Evading Transparent Proxy Daemon
Transparent proxy server with wire-level fragmentation support
"""
...
class TransparentProxyDaemon:
    """Transparent proxy daemon with wire-level fragmentation"""
    ...
    def __init__(self, listen_port=1080, chunk_size=30, chunk_delay=0.005):
        ...
        self.logger.info(f"Proxy daemon initialized: port={listen_port}, chunk_size={chunk_size}, chunk_delay={chunk_delay}")
    ...
    async def handle_client(self, reader, writer):
        ...
        try:
            # Extract original destination using getsockopt
            original_dst = self.extract_original_destination(writer)
            if not original_dst:
    ...
    def extract_original_destination(self, writer):
        """Extract original destination using socket.getsockopt"""
        try:
            sock = writer.get_extra_info('socket')
            if not sock:
                return None
            
            # Get original destination using Linux SO_ORIGINAL_DST
            orig_dst = sock.getsockopt(socket.SOL_IP, socket.SO_ORIGINAL_DST, 16)
            if not orig_dst:
                return None
            
            # Parse the destination address
            if len(orig_dst) == 4:  # IPv4
                ip_bytes = orig_dst
                ip = socket.inet_ntoa(ip_bytes)
                port = struct.unpack('!H', orig_dst[4:6])[0]
                
                return f"{ip}:{port}"
            else:
                self.logger.warning(f"Unsupported destination format: {len(orig_dst)} bytes")
                return None
    ...
    async def connect_upstream(self, destination):
        ...
        try:
            ...
            # Store socket reference for destination extraction
            writer.get_extra_info()['socket'] = writer.get_extra_info().get_extra_info('socket')
    ...
    async def forward_client_to_upstream(self, reader, writer, upstream_writer):
        ...
        try:
            while True:
                ...
                # Apply wire fragmentation to outgoing data
    ...
    async def start_server(self):
        ...
        self.logger.info(f"Starting transparent proxy on port {self.listen_port}")
        ...
        self.logger.info(f"Transparent proxy server listening on 0.0.0.0:{self.listen_port}")
        ...
        self.running = True
        self.logger.info("Transparent proxy daemon started successfully")
    ...
    async def stop_server(self):
        ...
        self.running = False
        self.logger.info("Stopping transparent proxy daemon")
        
        if hasattr(self, 'server') and self.server:
            self.server.close()
            self.logger.info("Transparent proxy daemon stopped")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DPI-Evading Transparent Proxy Daemon')
