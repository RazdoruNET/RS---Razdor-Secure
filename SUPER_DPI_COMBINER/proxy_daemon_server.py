#!/usr/bin/env python3
"""
DPI-Evading Transparent Proxy Daemon - Серверная версия
...
class ServerProxyDaemon:
    """Серверная версия прозрачного прокси с wire fragmentation"""
    ...
    def __init__(self, listen_port=1080, chunk_size=30, chunk_delay=0.005):
        ...
        self.logger.info(f"Server proxy daemon initialized: port={listen_port}, chunk_size={chunk_size}, chunk_delay={chunk_delay}")
    ...
    async def start_server(self):
        ...
        self.logger.info(f"Starting server proxy daemon on port {self.listen_port}")
        ...
        self.logger.info(f"Server proxy daemon listening on 0.0.0.0:{self.listen_port}")
        ...
        self.running = True
        self.logger.info("Server proxy daemon started successfully")
    ...
    async def stop_server(self):
        ...
        self.running = False
        self.logger.info("Stopping server proxy daemon")
        
        if hasattr(self, 'server') and self.server:
            self.server.close()
            self.logger.info("Server proxy daemon stopped")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DPI-Evading Server Proxy Daemon')
    ...
    # Create and start server daemon
    daemon = ServerProxyDaemon(
